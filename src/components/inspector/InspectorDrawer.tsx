...
export function InspectorDrawer() {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-zinc-950 border-l border-zinc-200 dark:border-zinc-800/80">
      {/* Header Tabs */}
      <div className="h-11 flex items-center justify-between px-3 border-b border-zinc-200 dark:border-zinc-800/80 bg-zinc-50 dark:bg-zinc-900/60">
        <div className="flex items-center gap-1 overflow-x-auto">
          {/* Plan Review Tab */}
          <button
            onClick={() => setInspectorTab('plan')}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium transition cursor-pointer shrink-0 ${
              inspectorTab === 'plan'
                ? 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-700/60 shadow-2xs'
          ...
          >
            ...
            {rawCount > 0 && (
              <span className="text-[10px] px-1 py-0 bg-zinc-200/80 dark:bg-zinc-800 rounded font-mono text-zinc-600 dark:text-zinc-400">
                {rawCount}
              </span>
            )}
          </button>
